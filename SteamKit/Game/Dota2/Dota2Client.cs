...
namespace SteamKit.Game.Dota2
{
    ...
    /// Dota2Client
    /// </summary>
    public class Dota2Client : GameClient
    {
        ...
        /// <returns></returns>
        public Dota2Client WithGCConnectionStatus(EventHandler<CMsgConnectionStatus> callback)
        {
        ...
        /// <param name="callback">Callback</param>
        /// <returns></returns>
        public Dota2Client RegistItemAcquired(InventoryChangedEventHandler callback)
        {
        ...
        /// <param name="callback">Callback</param>
        /// <returns></returns>
        public Dota2Client RegistItemChanged(InventoryChangedEventHandler callback)
        {
        ...
        /// <param name="callback">Callback</param>
        /// <returns></returns>
        public Dota2Client RegistItemRemoved(InventoryChangedEventHandler callback)
        {
            ...
            using (await loginGameJob.Lock.LockAsync(cancellationToken))
            {
                ...
                TimerCallback timerCallback = (obj) =>
                {
                    try
                    {
                        ...
                        var clientMsg = new GCClientProtoBufMsg<Client.Model.GC.Dota2.CMsgClientHello>((uint)EGCBaseClientMsg.k_EMsgGCClientHello);
                        clientMsg.Body.version = Version;
                        clientMsg.Body.engine = ESourceEngine.k_ESE_Source2;
        ...
        private void HandleSOMultipleObjects(CMsgSOMultipleObjects multipleObjects)
        {
            using (soMsgHandleLock.Lock(TimeSpan.FromSeconds(5)))
            {
                ...
                var soEconItems = multipleObjects.objects_added.Where(c => c.type_id == CSOEconItemTypeId).ToList();
                if (soEconItems.Any())
                {
                    foreach (var obj in soEconItems)
                    {
                        ProcessSOEconItemFromSOMsg(ESOMsg.k_ESOMsg_Create, obj.type_id, obj.object_data);
                    }
                }

                soEconItems = multipleObjects.objects_modified.Where(c => c.type_id == CSOEconItemTypeId).ToList();
                if (soEconItems.Any())
                {
                    foreach (var obj in soEconItems)
                    {
                        ProcessSOEconItemFromSOMsg(ESOMsg.k_ESOMsg_Update, obj.type_id, obj.object_data);
                    }
                }

                soEconItems = multipleObjects.objects_removed.Where(c => c.type_id == CSOEconItemTypeId).ToList();
                if (soEconItems.Any())
                {
                    foreach (var obj in soEconItems)
                    {
                        ProcessSOEconItemFromSOMsg(ESOMsg.k_ESOMsg_Destroy, obj.type_id, obj.object_data);
                    }
                }
            }
        }
        ...
        private void ProcessSOEconItemFromSOMsg(CMsgSOMultipleObjects objects)
        {
            List<CSOEconItem> modifiedInventories = new List<CSOEconItem>();
            List<CSOEconItem> addedInventories = new List<CSOEconItem>();
            List<CSOEconItem> removedInventories = new List<CSOEconItem>();

            foreach (var obj in objects.objects_modified)
            {
            ...
            foreach (var obj in objects.objects_added)
            {
                var inventory = ProcessSOEconItem(obj.type_id, obj.object_data);
                if (inventory == null)
                {
            ...
            foreach (var obj in objects.objects_removed)
            {
                var inventory = ProcessSOEconItem(obj.type_id, obj.object_data);
                if (inventory == null)
                {
                ...
                removedInventories.Add(inventory);

                inventories.RemoveAll(c => c.id == inventory.id);
            }

            if (modifiedInventories.Any() && itemHandleCallbasks.TryGetValue(ESOMsg.k_ESOMsg_Update, out var modifiedCallbask))
            {
                modifiedCallbask?.Invoke(modifiedInventories);
            }
            if (addedInventories.Any() && itemHandleCallbasks.TryGetValue(ESOMsg.k_ESOMsg_Create, out var addedCallbask))
            {
                addedCallbask?.Invoke(addedInventories);
            }
            if (removedInventories.Any() && itemHandleCallbasks.TryGetValue(ESOMsg.k_ESOMsg_Destroy, out var removedCallbask))
            {
                removedCallbask?.Invoke(removedInventories);
            }
        }
